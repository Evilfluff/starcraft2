...
from sc2.constants import NEXUS, PROBE, PYLON, ASSIMILATOR, GATEWAY, FORGE, \
 CYBERNETICSCORE, STARGATE, VOIDRAY, SCV, DRONE, ROBOTICSFACILITY, OBSERVER, \
 ZEALOT, STALKER, AbilityId, TWILIGHTCOUNCIL, PHOTONCANNON
import random
import cv2
import numpy as np
import os
...
class SentdeBot(sc2.BotAI):
    ...
    async def upgrade(self):
        pylon = self.units(PYLON).ready.noqueue.random
        #if len(self.units(ZEALOT)) < (8 - self.time): # how we can phase out zealots over time?
        if self.units(FORGE).ready.exists and self.units(FORGE).ready.noqueue:
                    for lab in self.units(FORGE).ready:
                        abilities = await self.get_available_abilities(lab)
                        if self.can_afford(random.choice(abilities)) and self.units(FORGE).ready.noqueue:
                            await self.do(lab(random.choice(abilities)))

    async def build_gateway(self):
        #if len(self.units(GATEWAY)) < 5:
        pylon = self.units(PYLON).ready.noqueue.random
        if self.can_afford(GATEWAY) and len(self.units(GATEWAY)) < 15:
            await self.build(GATEWAY, near=pylon.position.towards(self.game_info.map_center, 5))

    async def build_twilight(self):
        pylon = self.units(PYLON).ready.noqueue.random
        if self.can_afford(PHOTONCANNON) and len(self.units(PHOTONCANNON)) < 12:
    ...
    async def build_stargate(self):
        if self.units(PYLON).ready.exists:
            ...
            if self.units(CYBERNETICSCORE).ready.exists:
                    if self.can_afford(FORGE) and len(self.units(FORGE)) < 1:
                        await self.build(FORGE, near=pylon.position.towards(self.game_info.map_center, 5))
                    else:
                        if self.can_afford(PHOTONCANNON) and len(self.units(NEXUS)) > 1:
                            await self.build(PHOTONCANNON, near=pylon)
    ...
    async def attack_known_enemy_structure(self):
        if len(self.known_enemy_structures) > 0:
            target = self.known_enemy_units.closest_to(random.choice(self.known_enemy_structures))
            for u in self.units(VOIDRAY).idle:
                await self.do(u.attack(target))
            for u in self.units(STALKER).idle:
                await self.do(u.attack(target))
    ...
    async def do_something(self):
        ...
        if self.time > self.do_something_after:
            if self.use_model:
                ...
                choice_weights = 1*[0]+zealot_weight*[1]+gateway_weight*[2]+voidray_weight*[3]+stalker_weight*[4]+worker_weight*[5]+1*[6]+stargate_weight*[7]+pylon_weight*[8]+1*[9]+1*[10]+1*[11]+1*[12]+1*[13]+scout_weight*[14]
